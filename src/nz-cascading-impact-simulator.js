...
// ============ AF8 LOCATOR ============
// "400km of the Alpine Fault from Fiordland to Kelly Range" is hard to picture
// without seeing it, and af8 is the only scenario operating at a scale where a
// map is the information rather than decoration. Inline SVG rather than a raster
// or a tile service: it is 3.6 KB, needs no network, and themes with the app.
//
// Generated by tools/build-af8-map.js from real coordinates - edit that, not this.
// The builder asserts every point of the trace sits inland of the West Coast,
// because the fault runs along the range front parallel to the coast and a trace
// that wanders offshore is the one error a New Zealand audience would spot.
var AF8_LOCATOR_SVG =
  '<svg class="af8map" viewBox="-18.8 -21.4 406.9 433.4" role="img" aria-label="Map of the South Island of New Zealand showing about 349 kilometres of the Alpine Fault ruptured from Fiordland to the Kelly Range, with the epicentre near Milford Sound">' +
  '  <path class="af8map-coast" d="M281.6 12.6 L296.3 15 L289.4 30 L294.6 28.8 L298.5 45 L307.6 58.2 L324.1 46.8 L353.3 40.8 L354.1 47.4 L344.6 61.2 L350.6 85.2 L339.8 100.8 L326.3 127.8 L315.9 139.2 L297.6 165 L284.6 174 L283.7 192 L286.8 198 L299.8 207 L293.7 214.8 L282.4 211.2 L269.4 216 L260.7 216 L243.3 225 L219.4 246 L207.3 288 L200.7 310.2 L196.4 336 L173.8 345 L157.3 369 L113 376.8 L93 378 L86.9 367.2 L79.1 363 L58.7 354 L17.4 351.6 L15.2 327 L30.4 300 L71.7 258 L80.4 237 L84.7 238.2 L94.3 222.6 L105.1 220.2 L123.4 214.8 L135.6 207 L147.7 198 L158.6 189 L172.5 175.8 L193.4 159 L207.3 145.2 L217.7 129 L229 87 L256.8 57 L261.6 28.8 Z"/>' +
  '  <path class="af8map-fault-rest" d="M222.5 145.2 L235.5 132 L247.7 120 L267.2 102 L284.6 87 L302 75"/>' +
  '  <path class="af8map-fault" d="M74.7 258 L89.1 243 L102.1 225 L121.7 217.2 L134.7 211.2 L147.7 202.8 L158.6 195 L167.3 189 L174.7 183.6 L183.4 177 L192.1 169.8 L202.9 159 L210.7 151.8 L222.5 145.2"/>' +
  '  <g>' +
  '    <g class="af8map-epi"><circle cx="75.2" cy="262.2" r="9"/><circle cx="75.2" cy="262.2" r="5"/><circle cx="75.2" cy="262.2" r="2.4" class="af8map-epi-core"/></g>' +
  '    <text class="af8map-label epicentre" x="83.2" y="266.2" text-anchor="start">Milford Sound</text>' +
  '    <circle class="af8map-dot rupture" cx="222.5" cy="145.2" r="2.6"/>' +
  '    <text class="af8map-label rupture" x="230.5" y="156.2" text-anchor="start">Kelly Range</text>' +
  '    <circle class="af8map-dot town" cx="106.9" cy="283.8" r="2.6"/>' +
  '    <text class="af8map-label town" x="113.9" y="287.8" text-anchor="start">Queenstown</text>' +
  '    <circle class="af8map-dot town" cx="123.4" cy="214.8" r="2.6"/>' +
  '    <text class="af8map-label town" x="116.4" y="218.8" text-anchor="end">Haast</text>' +
  '    <circle class="af8map-dot town" cx="172.9" cy="185.4" r="2.6"/>' +
  '    <text class="af8map-label town" x="165.9" y="186.4" text-anchor="end">Franz Josef</text>' +
  '    <circle class="af8map-dot town" cx="207.3" cy="145.2" r="2.6"/>' +
  '    <text class="af8map-label town" x="200.3" y="149.2" text-anchor="end">Hokitika</text>' +
  '    <circle class="af8map-dot town" cx="217.7" cy="129" r="2.6"/>' +
  '    <text class="af8map-label town" x="210.7" y="125.0" text-anchor="end">Greymouth</text>' +
  '    <circle class="af8map-dot town" cx="234.6" cy="87" r="2.6"/>' +
  '    <text class="af8map-label town" x="227.6" y="87.0" text-anchor="end">Westport</text>' +
  '    <circle class="af8map-dot town" cx="307.6" cy="58.2" r="2.6"/>' +
  '    <text class="af8map-label town" x="314.6" y="53.2" text-anchor="start">Nelson</text>' +
  '    <circle class="af8map-dot town" cx="279.4" cy="193.8" r="2.6"/>' +
  '    <text class="af8map-label town" x="286.4" y="190.8" text-anchor="start">Christchurch</text>' +
  '    <circle class="af8map-dot port" cx="283.3" cy="198" r="2.6"/>' +
  '    <text class="af8map-label port" x="290.3" y="206.0" text-anchor="start">Lyttelton</text>' +
  '  </g>' +
  '  <g class="af8map-key" transform="translate(-2.8,366.0)">' +
...
var SCENARIO_CONFIGS = {
  af8: {
    ...
    card: {
      ...
      locator: function () { return AF8_LOCATOR_SVG; }
    },
    classification: 'R3',
    classCSS: 'r3',
    classText: 'R3 \u25B2',
...
var AF8_EVENTS = [
  ...
  { time: 590, type: 'decision', tag: 'CONTROLLER', title: 'Grey Base Hospital \u2014 Immediate Response', body: 'Grey Base Hospital has 8 ventilator-dependent patients and 12 hours of generator fuel. A helicopter fuel delivery requires a minimum 2-hour round trip from Christchurch and uses significant fuel itself. Medical evacuation of 8 ventilator patients by helicopter would require 4 dedicated flights with medical crews. Or you can attempt both \u2014 fuel delivery AND patient preparation for evacuation if power fails.',
    ...
    options: [
      ...
      { key: 'C', label: 'Deliver fuel now to buy time while preparing evacuation as a contingency', desc: 'Belt and suspenders. Fuel delivery keeps the lights on while evacuation is prepared as backup. Resource-intensive but protects against single-point failure.', effect: { score: 4, helicopters: -2 } },
...
function addEventToFeed(event) {
  ...
  // A scenario may also carry a locator map, on the same opening-entry rule.
  var locator = (!feed.children.length && sceneCard && sceneCard.locator)
    ? '<figure class="event-map">' + sceneCard.locator() +
      '<figcaption>Rupture extent and the places this response turns on.</figcaption></figure>'
    : '';
...
function makeDecision(key) {
  ...
  // Everything this decision moves is collected here and posted as one card
  // once the consequences have settled.
  beginChangeLedger();

  if (option.effect) {
    if (option.effect.score) {
      applyScoreDelta(option.effect.score);
    }
  ...
  if (wasNoise) {
    ...
    if (noiseScore !== 0) {
      ...
      // Apply directly
      var oldUtil = GameState.utilities[randomUtil].value;
      GameState.utilities[randomUtil].value = Math.max(0, Math.min(100, oldUtil + noiseDelta));
      recordChange(GameState.utilities[randomUtil].label,
        GameState.utilities[randomUtil].value - oldUtil, GameState.utilities[randomUtil].value, '%');
      ...
      if (panel) {
        ...
        if (item) {
          ...
          showValueDelta(pctSpan, nv - oldUtil);
      ...
      if (softPanel) {
        ...
        if (softItem) {
          ...
          showValueDelta(sp, sv - oldSoft);
          triggerDashAnimation(softItem, softDelta < 0 ? 'down' : 'up');
        }
      }
      var newSoft = GameState.softMetrics[randomSoft].value;
      recordChange(GameState.softMetrics[randomSoft].label, newSoft - oldSoft, newSoft, '');
    }
    // Noise never reaches applyConsequences, so it posts its own card.
    flushChangeLedger();
    processNextEvent();
  } else {
...
function applyConsequences(decId, key) {
  ...
  // Score, utilities and soft metrics have all moved by here, so the ledger is
  // complete and goes to the feed as one card - before any SITUATION CHANGE or
  // consequence inject, which are separate things that happen next.
  flushChangeLedger();
  ...
  if (consequence.inject) {
    ...
    eventTimer = setTimeout(function() {
      ...
      // Score penalty from the inject itself. This lands seconds after the
      // decision card, so it gets its own ledger and its own feed entry rather
      // than moving the score with nothing to explain it.
      if (inj.scorePenalty) {
        beginChangeLedger();
        applyScoreDelta(inj.scorePenalty);
        flushChangeLedger('EFFECT OF THIS CONSEQUENCE');
      }
      GameState.eventIndex++;
      processNextEvent();
    }, 3000);
...
// ============ NUMERIC CHANGE FEEDBACK ============
// Every number a decision moves used to change in silence: the meters flashed a
// coloured border for three seconds without ever saying by how much, and the
// Effectiveness score - the one headline number - had no signal at all.
//
// Three surfaces, fed from one ledger so they cannot disagree:
//   1. a floating delta chip on the row that moved     (immediate, in place)
//   2. the same chip on the Effectiveness readout      (the headline number)
//   3. one consolidated card in the event feed         (durable, scrollable)
var changeLedger = null;

function beginChangeLedger() { changeLedger = []; }

function recordChange(label, delta, now, unit) {
  if (!changeLedger || !delta) return;
  changeLedger.push({ label: label, delta: delta, now: now, unit: unit || '' });
}

// A transient "+8" / "-15" beside a value that just moved. Absolutely positioned
// against the row so it cannot reflow the panel, and given its own background so
// it stays legible over whatever it briefly covers.
function showValueDelta(el, delta) {
  if (!delta || !el || typeof document === 'undefined') return;
  var row = el.closest ? el.closest('.sit-item, .status-item') : null;
  if (!row) return;
  var old = row.querySelector('.val-delta');
  if (old) old.parentNode.removeChild(old);
  var chip = document.createElement('span');
  chip.className = 'val-delta ' + (delta > 0 ? 'up' : 'down');
...
// The single entry point for every score move. There are two paths - the
// decision itself, and a consequence's scorePenalty three seconds later - and
// both used to mutate the score independently and silently. A signal that
// covered only the first would teach the player to trust it, then lie.
function applyScoreDelta(delta) {
  if (!delta) return;
  var before = GameState.score;
  GameState.score = Math.max(0, Math.min(100, GameState.score + delta));
  // Report the move that actually landed, not the one that was asked for:
  // clamping at 0 and 100 can make them differ.
  var actual = GameState.score - before;
  if (typeof document !== 'undefined') {
    var el = document.getElementById('val-score');
    if (el) {
      el.textContent = GameState.score + '%';
      showValueDelta(el, actual);
    }
  }
  recordChange('Effectiveness', actual, GameState.score, '%');
}

function flushChangeLedger(heading) {
  var changes = changeLedger;
  changeLedger = null;
  if (!changes || !changes.length || typeof document === 'undefined') return;
  var feed = document.getElementById('event-feed');
  if (!feed) return;
  var rows = '';
  for (var i = 0; i < changes.length; i++) {
    var c = changes[i];
    var up = c.delta > 0;
    rows += '<div class="metric-change">' +
      '<span class="metric-change-label">' + c.label + '</span>' +
      '<span class="metric-change-delta ' + (up ? 'up' : 'down') + '">' +
        (up ? '+' : '') + c.delta + c.unit + '</span>' +
      '<span class="metric-change-now">now ' + c.now + c.unit + '</span>' +
      '</div>';
  }
  var entry = document.createElement('div');
  entry.className = 'event-entry';
  entry.innerHTML =
    '<div class="event-timestamp">' + (heading || 'EFFECT OF YOUR DECISION') + '</div>' +
    '<div class="metric-changes">' + rows + '</div>';
  feed.appendChild(entry);
  feed.scrollTop = feed.scrollHeight;
}
...
function applyUtilityEffects(decId, key) {
  ...
  for (var c = 0; c < changed.length; c++) {
    ...
    // The clamp means the authored delta and the move that landed can differ.
    var landed = ch.newVal - ch.oldVal;
    showValueDelta(pctSpan, landed);
    recordChange(GameState.utilities[ch.key].label, landed, ch.newVal, '%');
...
var FACILITATOR_NOTES = {
  ...
  'eoc_level': {
    ...
    teachingNote: 'This decision tests whether players understand the Scenario 2 context from the CCC Earthquake Plan. The key insight is that JESP co-location is strongly preferred when the primary challenge is coordination with the Group ECC, not independent city-level response. Activating at JESP alongside the Group ECC mirrors the pragmatic approach recommended in the plan. Splitting the team across both sites is defensible but risks under-resourcing local community needs.',
    discussionPrompts: [
  ...
  'airport_priority': {
    ...
    teachingNote: 'Christchurch Airport is designated strategic lifeline infrastructure. Full commercial reopening creates an airspace management crisis that is entirely predictable and avoidable. The key teaching point is that "projecting normalcy" is not a valid emergency management objective when it conflicts with response operations. Emergency-only operations is doctrinally correct, but a phased reopening is the most operationally realistic.',
    discussionPrompts: [
  ...
  'welfare_split': {
    ...
    teachingNote: 'This is the central tension of the Canterbury Scenario 2. The "right" answer - backing marae and Community Hubs for local welfare - requires trusting community self-organisation, which runs against the instinct to control everything from the EOC. Players who choose the 70/30 support split are technically correct about the greater need but will face political backlash. The discussion should focus on how institutional support enables rather than replaces community-led response.',
    discussionPrompts: [
  ...
  'fuel_priority': {
    ...
    teachingNote: 'This decision tests whether players understand the limits of their authority. Canterbury is a distribution node, not the decision-maker on inter-regional fuel allocation. Letting the National Controller decide allocation is correct because it reflects the CDEM Act hierarchy. Players who self-prioritise Canterbury\u2019s own facilities are overstepping their role in a national emergency.',
    discussionPrompts: [
  ...
  'psychosocial': {
    ...
    teachingNote: 'Canterbury\u2019s psychosocial impact is disproportionate to physical damage because of CES trauma reactivation. This tests whether players can recognise when the dominant response need is psychological rather than physical. A PIM pivot alone backfires because messaging that says "this isn\u2019t 2011" feels dismissive to people reliving 2011. The multi-layered approach is resource-intensive but addresses the full spectrum.',
    discussionPrompts: [
  ...
  'water_response': {
    ...
    teachingNote: 'The key tension: Canterbury residents can boil tap water. West Coast residents have no water at all. Regional mutual aid - neighbouring districts covering local need - is the best answer because it solves the local need through district-level cooperation while freeing CCC tankers for the greater need. This tests understanding of mutual aid provisions and proportionate resource allocation.',
    discussionPrompts: [
  ...
  'nzdf_staging': {
    ...
    teachingNote: 'NZDF operates within CDEM framework during emergencies, not independently. Letting NZDF self-coordinate from Burnham is a predictable failure because military logistics planning without CDEM coordination creates conflicts at chokepoints like the Lyttelton Tunnel. The embedded liaison officer is correct because it maintains the CDEM coordination function without overcomplicating the structure.',
    discussionPrompts: [
  ...
  'accommodation': {
    ...
    teachingNote: 'The A&P Showgrounds is the institutional default but creates a "refugee camp" dynamic that is both inadequate for extended stays and politically damaging. The distributed model is harder to manage but uses all available capacity and keeps people in more dignified settings. D4H welfare registration is the enabler that makes distributed accommodation trackable.',
    discussionPrompts: [
  ...
  'movement_control': {
    ...
    teachingNote: 'Canterbury is not the disaster zone \u2014 it\u2019s the logistics hub. Full movement control around the airport and port is disproportionate and will generate backlash. No formal control allows response convoys to be delayed by civilian traffic. Dedicated response corridors are the proportionate middle ground that keeps logistics flowing without restricting civilian movement unnecessarily.',
    discussionPrompts: [
  ...
  'media_management': {
    ...
    teachingNote: 'Media access is a legitimate need \u2014 public information serves accountability and community resilience. But uncontrolled media helicopter flights conflict with response operations. The embedded pool is standard practice in military and civil defence contexts. The PIM media centre is proactive and feeds the media without diverting response resources.',
    discussionPrompts: [
  ...
  'port_resilience': {
    ...
    teachingNote: 'Lyttelton Port is the South Island\u2019s single point of failure for fuel. This decision tests strategic thinking beyond the immediate crisis. Timaru diversification is the strongest answer because it addresses systemic vulnerability. Players who accept the risk are being operationally lazy.',
    discussionPrompts: [
  ...
  'declaration': {
    ...
    teachingNote: 'This is a nuanced legal question. The national declaration already covers Canterbury. A separate local declaration is legally valid but may be unnecessary. Operating under the national declaration is arguably the most legally sound. A local declaration gives additional powers but risks community alarm. The discussion should focus on proportionality and whether additional powers are actually needed.',
    discussionPrompts: [
      ...
      'Could a targeted geographic declaration, limited to the eastern suburbs and CBD cordon, actually be implemented?'
    ],
    bestPractice: 'B'
  },
  // ---- AF8 REGIONAL ----
  ...
  'af8_franz_josef': {
    ...
    teachingNote: 'This tests resource discipline. Committing every rotary asset saves Franz Josef but leaves zero capacity for everything else. A PIM broadcast alone is reckless because degraded comms make broadcast unreliable. Pairing the broadcast with targeted extraction is the balanced approach, using limited helicopters for vulnerable populations while leveraging community self-evacuation for the able-bodied. The consequence for committing the whole fleet is brutal but realistic \u2014 fleet exhaustion from a single commitment.',
    discussionPrompts: [
  ...
  'af8_queenstown': {
    ...
    teachingNote: 'This is the defining SAFER Framework decision. "No regrets" means pushing supplies before you have confirmed assessments. Waiting for the Queenstown Airport assessment is exactly the kind of cautious, assessment-driven approach that the SAFER Framework was designed to overcome. The fatal consequence for waiting is intentionally severe because it demonstrates why "no regrets" exists \u2014 people die while you are optimising logistics.',
    discussionPrompts: [
      ...
      'If Queenstown Airport had been usable, would waiting for the assessment have been right?'
    ],
    bestPractice: 'A'
  },
  'af8_fuel': {
    ...
    teachingNote: 'There is no good answer here. Every option has a life-safety downside. Hospitals and health facilities first is the least bad because it protects the greatest number of critical patients. Helicopter operations first is tempting for responders but hospital generator failure kills more people faster than grounded helicopters. This should generate the most heated discussion in any debrief.',
    discussionPrompts: [
  ...
  'af8_recon': {
    ...
    teachingNote: 'Zero-contact communities are the highest reconnaissance priority because you cannot assess their needs without reaching them. Infrastructure reconnaissance enables planning but does not save lives today. Focusing on zero-contact communities is correct because the SAFER Framework explicitly prioritises community contact over infrastructure assessment in the first 48 hours.',
    discussionPrompts: [
  ...
  'af8_hospital': {
    ...
    teachingNote: 'The belt-and-suspenders approach - fuel now, evacuation prepared - is correct because it protects against single-point failure. Fuel delivery alone leaves you with no backup if delivery fails. Evacuation alone is high-risk for ventilator patients in helicopter transport. The combined approach costs more resources but ensures patient survival under multiple failure scenarios.',
    discussionPrompts: [
  ...
  'af8_usar_deploy': {
    ...
    teachingNote: 'Holding USAR at the airport is the worst answer because survivability curves for trapped persons are steep \u2014 every hour of delay reduces survival probability. Greymouth is the strongest choice because it has the largest affected population and confirmed collapses. The frustrated USAR commander consequence for holding is based on real incidents where deployment delays generated international diplomatic friction.',
    discussionPrompts: [
  ...
  'af8_weather_window': {
    ...
    teachingNote: 'Pre-positioning is the most strategic choice because it positions resources where they will be needed during the weather window. Communities with supplies can shelter in place during bad weather. Putting every helicopter on evacuation is tempting but leaves nothing in position for the 12-18 hours when helicopters are grounded. This tests whether players can think beyond the immediate and plan for the next operational period.',
    discussionPrompts: [
  ...
  'af8_transition': {
    ...
    teachingNote: 'The overlapping shift is best practice because it preserves institutional knowledge during handover. Full rotation loses critical context. Retaining key personnel burns out your most important people. The 4-hour overlap costs double staffing but prevents the situational awareness gap that causes errors in the second operational period. This is a well-documented failure mode in prolonged emergencies.',
    discussionPrompts: [
...
function applySoftMetricEffects(decId, key) {
  ...
  for (var metric in effects) {
    ...
    var landed = nv - oldVal;
    recordChange(GameState.softMetrics[metric].label, landed, nv, '');
